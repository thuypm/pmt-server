...
export default class UserRepository extends BaseRepository<UserDocument>{
    ...
    async getNotificationPaging(query: any, page:number) {
        let data = await this.model.findOne(query).select({ notices: 1, _id: 1 });
        let len: number = data?.notices?.length;
        // let totalPage: number = Math.ceil(len / PAGE_SIZE);
        let responseData = (page) * PAGE_SIZE > len ? data?.notices?.slice((page - 1) * PAGE_SIZE, len) : data?.notices?.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);
        return {
