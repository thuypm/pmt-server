...
export class ExerciseService {
  ...
  async createExercise(exercise: CreateExerciseDto) {
    ...
    try {
      groupFind = await this.checkForbidden(exercise?.group_id?.toString(), this.groupRepo, null);
    } catch (error) {
      throw error
    }
  ...
  async updateExercise(id: string, exerciseEdit: ExerciseDto) {
    ...
    return await this.exerciseRepo.update({ _id: exerciseFind._id }, {
  ...
  async submitExercise(file: Express.Multer.File, exercise_id: string, user: UserDecodeToken) {
    let exerciseFind: ExerciseDto;
    try {
      exerciseFind = await this.checkForbidden(exercise_id, this.exerciseRepo, null);
  ...
  async evaluateExercise(id: string, user: UserDecodeToken, evaluateData: any) {
    ...
    return await this.exerciseRepo.update({ _id: exerciseFind._id, "list_submit._id": evaluateData?._id }, {
      $set: {
