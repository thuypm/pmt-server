...
export class ExerciseController {
  constructor(private readonly exerciseService: ExerciseService, private readonly authService: AuthService) {

  }
  @Get('')
  async getAllExercise(@Body() body: ExerciseDto, @Query() query: any, @Headers('authorization') auth: string) {
  ...
  @Post("")
  async createExercise(@Body() body: any, @Headers('authorization') auth: string) {
    let token = auth.replace('Bearer ', '');
    const owner: UserDecodeToken = await this.authService.decodedToken(token);
    let exerciseCreate: CreateExerciseDto = {
      _id: null,
      name: body?.name,
