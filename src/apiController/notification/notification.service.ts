...
export class NotificationService {
  ...
  async getAllNotification(user: UserDecodeToken, page: number) {
    return await this.userRepo.getNotificationPaging({ username: user.username }, page);
  }
  async handleReadNotice(user: UserDecodeToken, listNoticeId: Array<string>) {
    listNoticeId.forEach(async noticeId => {
      await this.userRepo.update({ _id: Types.ObjectId(user._id.toString()), 'notices.id': noticeId }, {
        $set: {
          'notices.$.status': true
        }
      }
  ...
  async handleReadAllNotice(user: UserDecodeToken) {
    return await this.userRepo.updateMany({ _id: Types.ObjectId(user._id.toString()), 'notices.id': { $regex: /-/, } }, {
      $set: {
        'notices.$[].status': true
      }
    }
